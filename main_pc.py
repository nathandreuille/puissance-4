...
def fct_classical_party ():
    ...
    can = Canvas(fen,height= 450, width=525, bg = "#000EEC") # Création d'un canvas de dimensions 700x600 avec une couleur de fond.
    can.pack()
    ...
    button_replay = Button(fen,text = "↻",font = "arial 25 bold",command=fct_replay).place(x=50,y=500) # Bouton pour revenir un coup en arrière.
    label_explication_replay = Label (fen,text = "revenir un coup en arrière",font = "arial 8 bold").place(x=10,y=550) # Affichage d'une explication pour le bouton "revenir un coup en arrière".
...
def checkerboard(): # fonction pour créer le damier du jeu (sous forme de dessin)
    ...
    x2 = 75# coordonnées en x du point droit du rectangle 
    y2 = 150# coordonnées en y du point droit du rectangle 
...
def player_action(event): # fonction qui permet d'interpreter les entrées utilisateur (les clics de souris des joueurs)
    ...
    col = X//75 # on fait la division euclidienne pour recuperer la colone du clic qui sont numérotées de 1 à 6
    line = Y//75 # on fait la division euclidienne pour recuperer la ligne du clic qui sont numérotées de 1 à 7
    ...
    if player ==1 or player == 2:
        id_token = token(col*75,line*75,col*75+75,line*75+75,player) # on appelle la fonction pour créer un jeton et on change la couleur en fonction du "player" 
    ...
    if fct_equality(): # fonction qui test si il n'y a plus de place dans le damier et donc egalité
        ...
        label_equality.place(x = 100,y=285)
    ...
    if fct_line_winner() or fct_column_winner() or fct_diagonal_winner() : # controler si un joueur a alligné 4 jetons (en diagonale,ligne,colonne)
        if player == 1 or player == 2:
            label_winner = Label(fen,text = "Le joueur "+couleur_joueur[player]+" a gagné",font= "arial 20 bold",bg =couleur_code[player], fg ="white" )
            label_winner.place(x = 100,y=285)
    ...
    for col in range(7): # test colonne 
         for line in range(6): # test lignes 
            if checkerboard_list[col][line] == 1: # placer un pion jaune si il y a un 1
                id_token = token(col*75,line*75,col*75+75,line*75+75,1)
            elif checkerboard_list[col][line] == 2: # placer un pion rouge si il y a un 2
                id_token = token(col*75,line*75,col*75+75,line*75+75,2)
            else:
                pass
